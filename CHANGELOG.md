...
### Added
- RTBH (sub-project C, C1b): a `blackwall-rtbh` crate turning detected attacks into BGP blackholes. A pure `RtbhController` maps detection events (and manual triggers) to `/32`/`/128` blackhole announce/withdraw decisions with eligibility (own-prefix only), a concurrent-blackhole cap, and hold-down anti-flap; a thin `RtbhSink` implements `blackwall_flow::MitigationSink` and executes them via the C1a BGP speaker. A new `rtbh-bird` lab gate proves a blackhole `/32` (community `65535:666`) reaches real BIRD — the first detection→mitigation link. (Postgres persistence, an operator CLI, and daemon wiring are follow-ons.)
